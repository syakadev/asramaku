        ...
        <form action="{{ route('activities.update', $activity) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $activity->name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>

                <!-- Deskripsi -->
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                    <textarea id="description" name="description" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>{{ old('description', $activity->description) }}</textarea>
                </div>

                <!-- Lokasi -->
                <div>
                    <label for="location" class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
                    <input type="text" id="location" name="location" value="{{ old('location', $activity->location) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>

                <!-- Tanggal dan Waktu -->
                <div>
                    <label for="date" class="block text-sm font-medium text-gray-700 mb-1">Tanggal dan Waktu</label>
                    <input type="datetime-local" id="date" name="date" value="{{ old('date', \Carbon\Carbon::parse($activity->date)->format('Y-m-d\TH:i')) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
                ...
                <div>
                    ...
                    <select id="type" name="type" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" required>
                        <option value="1" {{ old('type', $activity->type) == '1' ? 'selected' : '' }}>Wajib</option>
                        <option value="0" {{ old('type', $activity->type) == '0' ? 'selected' : '' }}>Opsional</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="organizer_id" name="organizer_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" required>
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('organizer_id', $activity->organizer_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    @if ($activity->img)
                        <div class="mt-4">
                            ...
                            <img src="{{ asset('storage/images/' . $activity->img) }}" alt="Gambar Kegiatan" class="mt-2 h-24 w-24 object-cover rounded-md">
                        </div>
                    @endif
