...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Penilaian Kinerja</h1>
        ...
        <form action="{{ route('performs.update', $perform) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="user_id" name="user_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $perform->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="duty_schedule_id" name="duty_schedule_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" required>
                        ...
                        @foreach($dutySchedules as $schedule)
                            <option value="{{ $schedule->id }}" {{ old('duty_schedule_id', $perform->duty_schedule_id) == $schedule->id ? 'selected' : '' }}>{{ $schedule->duty->section }}</option>
                        @endforeach
                    </select>.
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="date" name="date" value="{{ old('date', $perform->date) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
